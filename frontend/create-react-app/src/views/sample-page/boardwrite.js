...
const BoardWrite = () => {
  ...
  useEffect(() => {
    ...
    const history = useHistory();
     ...
     // 작성 버튼 클릭 시 페이지 이동 처리
     const handleButtonClick = () => {
      history.push('/boardwrite'); // '/boardwrite' 경로로 페이지 이동
    };


  return (
    <MainCard title = {<span style={{ fontSize: '24px', fontWeight: 'bold' }} >글 작성</span>} style={{ marginLeft: '8px' }} secondary={<Button variant="outlined" onClick={() => handleButtonClick(item)} style={{ marginRight: '8px' }} >게시글 작성</Button>}>
